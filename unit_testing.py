			...
			for L in [0.5, 1, 10]:
				...
				for t in [0.0, 0.05, 1.0]:
					...
					for x in np.linspace(0, L, 30):
						...
						msg = "HeatFlow derived with respect to t yielded unexpected answer"
						assert abs(u_t(x, t) - u.t(x, t)) < tol, msg
						msg = "HeatFlow double derived with respect to x yielded unexpected answer"
	...
	for n in [1, 3]:
		for L in [0.5, 1]:
			...
			for dx in [0.05, 0.1, 0.2]:
				...
				x_full, t_full, u_full = pde.solve(T, dx)
				x_final, t_final, u_final = pde.solve(T, dx, sample="end")
				x_sample, t_sample, u_sample = pde.solve(T, dx, sample=2)
			...
			dx = 1e-3
			T = 1e-4
			x, t_full, u_full = pde.solve(T, dx)
			x, t_final, u_final = pde.solve(T, dx, sample="end")
			x, t_sample, u_sample = pde.solve(T, dx, sample=2)
